...
public:
    ...
    Background(SDL_Renderer* _renderer, string _graphic)
    {
        ...
        if (_graphic == "wall")
        {
            backgroundTexture = SDL_CreateTextureFromSurface(_renderer, wallSurface);
        }

        if (_graphic == "grass")
        {
            backgroundTexture = SDL_CreateTextureFromSurface(_renderer, grassSurface);
        }
...
public:
    ...
    SpriteBatch(SDL_Renderer* _renderer)
    {
        //Constructor.
    ...
    void createRandomSprite(SDL_Renderer* _renderer , SpriteBatch* _spriteBatch ,string _type, int _count)
    {
 
        for (int x = 0; x < _count; x++)
        {
            Sprite sprite(_renderer, _type, resolution_x / 2, resolution_y / 2);
    ...
    void update(SDL_Renderer* _renderer, ControlInput _controlInput, SoundEngine _soundEngine)
    {
        ...
        if (!spriteMatrix.empty() && paused == false)
        {
            ...
            for (unsigned int x = 0; x < spriteMatrix.size(); x++)
            {
                ...
                spriteMatrix.at(x).update(_renderer, _controlInput,_soundEngine);
            }
        }
    }
...
int main(int argc, char** args)
{
    ...
    spriteBatch.createRandomSprite(renderer, &spriteBatch, "slime", 20);
    ...
    Background backGround(renderer,"wall");
    ...
    while (!quit)
    {
        ...
        spriteBatch.update(renderer, controlInput, soundEngine);
